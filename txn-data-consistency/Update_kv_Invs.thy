theory Update_kv_Invs
  imports CCv_Eiger_Port_plus_proof
begin
...
next
  ...
  next
    ...
    then show ?case apply (simp add: Rtxn_RegK_in_rs_def tps_trans_defs)
      by (metis Kvt_map_t_Committed_def is_committed.elims(2) is_prepared.simps(3)
          reach_kvt_map_t_committed)
...
lemma read_done_new_read:
  ...
  using image_eqI apply blast
  subgoal apply (rule image_eqI, auto) (* rtxn_regk ?*) sorry
