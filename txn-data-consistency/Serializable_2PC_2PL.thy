...
definition abort where \<comment>\<open>Locks released (aborted)\<close>
  "abort s k s' t \<equiv>
    ...
    \<and> cl_state (cls s (get_cl_txn t)) = cl_aborted
    \<and> svr_vl (svrs s' k) = svr_vl (svrs s k)
...
definition cl_commit where
  "cl_commit s s' cl sn u'' F \<equiv>
    sn = cl_sn (cls s cl) \<and>
    u'' = (\<lambda>k. full_view (kvs_of_gs s k)) \<and>
    F = (\<lambda>k. svr_key_fp (svrs s k) (get_txn_cl cl s)) \<and>
    cl_state (cls s cl) = cl_prepared \<and>
    (\<forall>k. is_locked (svr_state (svrs s k) (get_txn_cl cl s))) \<and>
...
definition cl_abort where
  "cl_abort s s' cl \<equiv>
    cl_state (cls s cl) = cl_prepared
    \<and> (\<exists>k. svr_state (svrs s k) (get_txn_cl cl s) = notokay)
    \<and> (\<forall>k. svr_state (svrs s k) (get_txn_cl cl s) \<in> {read_lock, write_lock, no_lock, notokay})
...
definition cl_ready_c where
  "cl_ready_c s s' cl \<equiv>
    cl_state (cls s cl) = cl_committed
    \<and> (\<forall>k. svr_state (svrs s k) (get_txn_cl cl s) = committed)
...
definition cl_ready_a where
  "cl_ready_a s s' cl \<equiv>
    cl_state (cls s cl) = cl_aborted
    \<and> (\<forall>k. svr_state (svrs s k) (get_txn_cl cl s) = aborted)
    \<and> cl_state (cls s' cl) = cl_init
    \<and> cl_sn (cls s' cl) = Suc (cl_sn (cls s cl))
    \<and> cl_view (cls s' cl) = cl_view (cls s cl)
    \<and> svr_cl_cl'_unchanged cl s s'"
...
fun gs_trans :: "'v global_conf \<Rightarrow> 'v ev \<Rightarrow> 'v global_conf \<Rightarrow> bool" where
  ...
  "gs_trans s (Cl_Commit cl sn u F) s' \<longleftrightarrow> cl_commit s s' cl sn u F" |
  "gs_trans s (Cl_Abort cl)         s' \<longleftrightarrow> cl_abort s s' cl" |
  "gs_trans s (Cl_ReadyC cl)        s' \<longleftrightarrow> cl_ready_c s s' cl" |
