...
definition committed_at where
  "committed_at wtxn_s t cts \<equiv> \<exists>v rs. wtxn_s t = Some (Commit cts v rs)"

definition at_cts :: "'v state_wtxn \<Rightarrow> tstmp \<Rightarrow> tstmp" where
  "at_cts wtxn_s rts \<equiv> GREATEST cts. cts \<le> rts \<and> (\<exists>t. committed_at wtxn_s t cts)"
...
definition at :: "'v state_wtxn \<Rightarrow> tstmp \<Rightarrow> txid" where
  "at wtxn_s rts \<equiv> SOME t. committed_at wtxn_s t (at_cts wtxn_s rts)"

lemma "get_ts_ver (the (wtxn_s (at wtxn_s rts))) = at_cts wtxn_s rts" oops

abbreviation has_newer_own_write :: "'v state_wtxn \<Rightarrow> tstmp \<Rightarrow> cl_id \<Rightarrow> bool" where
  "has_newer_own_write wtxn_s cts cl \<equiv> \<exists>t. get_cl_wtxn t = cl \<and> committed_at wtxn_s t cts"
...
definition newest_own_write :: "'v state_wtxn \<Rightarrow> tstmp \<Rightarrow> cl_id \<rightharpoonup> txid" where
  "newest_own_write wtxn_s ts cl \<equiv>
    (if (\<exists>cts. has_newer_own_write wtxn_s cts cl \<and> cts \<ge> ts)
     then Some (SOME t. \<exists>v rs. wtxn_s t = Some (Commit (GREATEST cts. has_newer_own_write wtxn_s cts cl \<and> cts \<ge> ts) v rs))
     else None)"
  ...
  "kvs_of_s s \<equiv>
    (\<lambda>k. map
      (\<lambda>t. case wtxn_state (svrs s k) t of
        Some (Prep ts v) \<Rightarrow> \<lparr>v_value = v, v_writer = t, v_readerset = {}\<rparr> |
        Some (Commit cts v rs) \<Rightarrow> \<lparr>v_value = v, v_writer = t, v_readerset = rs\<rparr>)
      (commit_order s k))"
