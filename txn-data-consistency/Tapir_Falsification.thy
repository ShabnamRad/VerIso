...
next
        ...
        next 
          ...
          proof -
            have a: "\<And>k v. F k R = Some v \<Longrightarrow>
              \<exists>t. svr_state (svrs gs k) (get_wtxn gs cl) = prepared ts (Some (v, t)) (F k W)"
              using cmt
  ...
  next
    ...
    then show ?case
      ...
      by (rule ext, auto simp add: sim_defs split: svr_state.split option.split)
  next
  ...
  next
    ...
    then show ?case
      apply (auto simp add: sim_def tapir_trans_defs)
      apply (rule ext, auto simp add: sim_defs split: svr_state.split option.split) sorry
  next
  ...
  next
    ...
    then show ?case
      apply (auto simp add: sim_defs tapir_trans_defs)
      apply (rule ext, auto split: svr_state.split) sorry
  next
