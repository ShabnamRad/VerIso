...
next
  ...
  next
    ...
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs TIDFutureKm_def)
      apply (metis state_km.distinct(3))
      apply (metis state_km.distinct(5))
      by (metis state_km.distinct(7))
  next
    case (Abort x91 x92)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs TIDFutureKm_def)
      apply (metis state_km.distinct(3))
      apply (metis state_km.distinct(5))
      apply (metis state_km.distinct(7))
      by (metis state_km.distinct(9))
  next
...
next
  ...
  proof (cases e)
    ...
    then show ?thesis using reach_trans
      ...
      by (metis state_km.distinct(11) state_km.distinct(13))
  next
    case (RLock x41 x42 x43)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs TIDPastKm_def)
      by (metis state_km.distinct(23) state_km.distinct(25))
  next
    case (WLock x51 x52 x53 x54)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs TIDPastKm_def)
...
next
  ...
  proof (induction e)
    ...
    then show ?case using reach_trans
      ...
      by (metis state_km.distinct(15) state_km.distinct(17))
  next
    case (RLock x1 x2 x3)
    then show ?case using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockInv_def)
      by (metis state_km.distinct(27))
  next
  ...
  next
    ...
    then show ?case using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockInv_def)
      by (metis state_km.distinct(30) state_km.distinct(38))
  next
    case (NOK x1 x2)
    then show ?case using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockInv_def)
      by (metis state_km.distinct(31) state_km.distinct(39))
      (*apply (metis state_km.distinct(31))
      apply (metis state_km.distinct(31))
      by (metis state_km.distinct(40))*)
  next
    case (Commit x1 x2)
    then show ?case using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockInv_def)
      apply (metis state_km.distinct(42))
      apply (metis state_km.distinct(34))
      by (metis state_km.distinct(34) state_km.distinct(42))
  next
    case (Abort x1 x2)
    then show ?case using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockInv_def)
      apply (metis state_km.distinct(44))
      apply (metis state_km.distinct(36))
      apply (metis state_km.distinct(36) state_km.distinct(44))
      by (metis state_km.distinct(35) state_km.distinct(44))
...
next
  ...
  next
    ...
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs WLockInv_def)
      by (metis state_km.distinct(37))
  next
    case (NOK x71 x72)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs WLockInv_def)
      by (metis state_km.distinct(39))+
  next
    case (Commit x81 x82)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs WLockInv_def)
      by (metis state_km.distinct(41))+
  next
    case (Abort x91 x92)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs WLockInv_def)
      by (metis state_km.distinct(43))+
...
lemma the_wr_tI:
  assumes "km_state (svrs s k) t = write_lock"
      and "WLockInv s k"
  shows "the_wr_t (km_state (svrs s k)) = t"
  using assms
  by blast
...
lemma read_only_update [simp]:
  assumes "\<And>t. km_state (svrs s k) t \<noteq> write_lock"
  shows "update_kv_all_txn tStm (km_state (svrs s k)) tFk vl =
         update_kv_reads_all_txn tStm (km_state (svrs s k)) tFk vl"
  using assms
...
lemma writer_update_before:
  ...
  shows "update_kv_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k)) tFk vl =
         update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k)) tFk vl"
  using assms
  ...
  by (metis state_tm.distinct(7) txid0.sel(2))
...
next
  ...
  next
    ...
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockFpInv_def)
      by (metis state_km.distinct(29))+
  next
    case (NOK x71 x72)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockFpInv_def)
      by (metis state_km.distinct(31))+
  next
    case (Commit x81 x82)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockFpInv_def)
      by (metis state_km.distinct(33))+
  next
    case (Abort x91 x92)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs RLockFpInv_def)
      by (metis state_km.distinct(35))+
...
next
  ...
  next
    ...
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs NoLockFpInv_def)
      apply (metis state_km.distinct(37), metis)
      by (metis state_km.distinct(37))+
  next
    case (NoLock x61 x62)
    then show ?thesis using reach_trans
  ...
  next
    ...
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs NoLockFpInv_def)
      by (metis state_km.distinct(45))+
  next
    case (Commit x81 x82)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs NoLockFpInv_def)
      by (metis state_km.distinct(47))+
  next
    case (Abort x91 x92)
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs NoLockFpInv_def)
      by (metis state_km.distinct(49))+
...
lemma the_wr_t_fp:
  ...
  shows "svr_key_fp (svrs s k) (the_wr_t (km_state (svrs s k))) W \<noteq> None"
...
lemma eligible_reads_km_inv:
  assumes "RLockInv s k"
    and "(km_state (svrs s k) t \<notin> {write_lock, read_lock} \<and>
          km_state (svrs s' k) t = km_state (svrs s k) t) \<or>
          tm_state (cls s (get_cl_txn t)) \<noteq> tm_committed"
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "
  eligible_reads (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s' k)) (svr_key_fp (svrs s' k)) t' =
  eligible_reads (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k)) (svr_key_fp (svrs s k)) t'"
  using assms apply (cases "t' = t"; simp add: km_unchanged_defs) by metis
...
lemma eligible_reads_read_lock_inv:
  assumes "RLockFpInv s k"
    ...
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "{t. eligible_reads (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k))
            (svr_key_fp (svrs s k)) t} =
          insert t {t. eligible_reads (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s' k))
            (svr_key_fp (svrs s k)) t}"
...
lemma update_kv_reads_commit_w_s_inv:
  assumes "RLockInv s k" and "WLockInv s k"
    ...
    and "tm_state (cls s (get_cl_txn t)) = tm_committed"
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) 
                (km_state (svrs s k)) (svr_key_fp (svrs s k)) vl =
...
lemma update_kv_reads_commit_w_s'_inv:
  assumes "RLockInv s k" and "WLockInv s k"
    ...
    and "km_state (svrs s' k) t = committed"
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s' k))
...
lemma eligible_reads_no_lock_inv:
  assumes "km_state (svrs s k) t = no_lock"
    and "km_state (svrs s' k) t = committed"
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "
  eligible_reads (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s' k)) (svr_key_fp (svrs s k)) t' =
  ...
  apply (metis state_km.distinct(33))
  apply (metis state_km.distinct(41))
  apply (metis state_km.distinct(29))
  by (metis state_km.distinct(37))
...
lemma update_kv_writes_km_inv:
  assumes "WLockInv s k"
    ...
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "update_kv_writes_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t)))
                (km_state (svrs s' k')) (svr_key_fp (svrs s' k')) vl =
               update_kv_writes_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t)))
                (km_state (svrs s k')) (svr_key_fp (svrs s k')) vl"
...
lemma update_kv_writes_commit_r_inv:
  assumes "RLockInv s k"
    ...
    and "km_state (svrs s' k) t = committed"
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "update_kv_writes_all_txn tStm (km_state (svrs s' k)) (svr_key_fp (svrs s k)) vl = 
         update_kv_writes_all_txn tStm (km_state (svrs s k)) (svr_key_fp (svrs s k)) vl"
...
lemma update_kv_writes_commit_w_s_inv:
  assumes "WLockInv s k"
    ...
    and "tm_state (cls s (get_cl_txn t)) = tm_committed"
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "update_kv_writes_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k))
...
lemma update_kv_writes_commit_w_s'_inv:
  assumes "WLockInv s k"
    ...
    and "km_state (svrs s' k) t = committed"
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "update_kv_writes_all_txn tStm (km_state (svrs s' k)) (svr_key_fp (svrs s k)) vl = vl"
  using assms apply (auto simp add: update_kv_writes_all_txn_def km_unchanged_defs)
  by (metis state_km.distinct(41) the_wr_tI)

lemma update_kv_writes_commit_n_inv:
  assumes "WLockInv s k"
    and "km_state (svrs s k) t = no_lock"
    and "km_state (svrs s' k) t = committed"
    and "tm_km_k'_t'_unchanged k s s' t"
  shows "update_kv_writes_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s' k))
           (svr_key_fp (svrs s k)) vl =
         update_kv_writes_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k))
           (svr_key_fp (svrs s k)) vl"
  using assms apply (auto simp add: update_kv_writes_all_txn_def)
  subgoal for t' apply (cases "t' = t"; simp add: km_unchanged_defs)
    by (smt (verit) state_km.distinct(41) theI the_wr_tI)
...
lemma eligible_reads_tm_inv:
  ...
  subgoal
    ...
    apply (metis txid0.collapse state_km.distinct(33))
    by (metis empty_iff insert_iff other_sn_idle state_km.distinct(3)
        state_km.distinct(33) state_km.distinct(35))
  subgoal
    apply (cases "get_cl_txn t = cl"; cases "get_sn_txn t = cl_sn (cls s cl)";
            simp add: tm_unchanged_defs)
    apply (metis txid0.collapse state_km.distinct(41))
    by (metis empty_iff insert_iff other_sn_idle state_km.distinct(41)
        state_km.distinct(43) state_km.distinct(5))
  done
...
lemma eligible_reads_tm_commit_no_lock_inv:
  ...
  by (metis empty_iff txid0.collapse insert_iff other_sn_idle
      state_km.distinct(29) state_km.distinct(3) state_km.distinct(33) state_km.distinct(35)
      state_km.distinct(37) state_km.distinct(41) state_km.distinct(43) state_km.distinct(5))
...
lemma update_kv_writes_tm_inv:
  ...
  subgoal for t
    ...
    apply (metis txid0.collapse state_km.distinct(41))
    by (metis ex_in_conv insertE other_sn_idle state_km.distinct(41)
        state_km.distinct(43) state_km.distinct(5)).
...
lemma kvs_of_gs_inv:
  ...
  proof (induction e)
    ...
    then show ?case using kvs_of_gs_km_inv[of s x1 s' x2]
      by (auto simp add: tps_trans_defs km_unchanged_defs dest!: svr_vl_eq_all_k)
  next
    case (RLock x1 x2 x3)
...
lemma tm_commit_expands_eligible_reads:
  assumes "tm_state (cls s' cl) = tm_committed"
    and "other_insts_unchanged cl (cls s) (cls s')"
  shows
  "{t. eligible_reads (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k)) (svr_key_fp (svrs s k)) t} \<subseteq>
   {t. eligible_reads (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s k)) (svr_key_fp (svrs s k)) t}"
  using assms by (auto simp add: other_insts_unchanged_def)
...
lemma tm_commit_expands_eligible_reads':
  assumes "NoLockFpInv s k" and "TIDPastKm s cl" and "TIDFutureKm s cl"
    ...
    and "other_insts_unchanged cl (cls s) (cls s')"
  shows "{x. eligible_reads (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s k))
    (svr_key_fp (svrs s k)) x} = (case svr_key_fp (svrs s k) (get_txn_cl cl s) R of
   None \<Rightarrow> {x. eligible_reads (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k))
     (svr_key_fp (svrs s k)) x}|
   Some _ \<Rightarrow> insert (get_txn_cl cl s) {x. eligible_reads (\<lambda>t. tm_state (cls s (get_cl_txn t)))
     (km_state (svrs s k)) (svr_key_fp (svrs s k)) x})"
  using assms
  ...
  subgoal for t apply (cases "get_cl_txn t = cl"; cases "get_sn_txn t = cl_sn (cls s cl)"; simp)
    ...
    by (metis emptyE insert_iff other_sn_idle state_km.distinct(3) state_km.distinct(33)
        state_km.distinct(35) state_km.distinct(41) state_km.distinct(43) state_km.distinct(5))
  subgoal for v t apply (cases "get_cl_txn t = cl"; cases "get_sn_txn t = cl_sn (cls s cl)"; simp)
    apply (metis txid0.collapse)
    by (metis empty_iff insertE other_sn_idle state_km.distinct(3) state_km.distinct(33)
        state_km.distinct(35) state_km.distinct(41) state_km.distinct(43) state_km.distinct(5))
  by auto
...
lemma tm_commit_updates_kv_reads:
  assumes "KVSNonEmp s"
    ...
    and "other_insts_unchanged cl (cls s) (cls s')"
  shows
  "update_kv_reads_all_txn (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s k))
      (svr_key_fp (svrs s k)) (svr_vl (svrs s k)) = 
   update_kv_reads_all_txn (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s k))
      (svr_key_fp (svrs s k))
        (update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k))
...
lemma tm_commit_updates_kv_reads':
  ...
  "update_kv_reads_all_txn (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s k))
   ...
   update_kv_reads (get_txn_cl cl s) (svr_key_fp (svrs s k) (get_txn_cl cl s))
     (full_view (update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k))
          (svr_key_fp (svrs s k)) (svr_vl (svrs s k))))
        (update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) (km_state (svrs s k))
          (svr_key_fp (svrs s k)) (svr_vl (svrs s k)))"
...
next
  ...
  next
    ...
    then show ?thesis using reach_trans svr_vl_read_commit_eq_lv[of s x81 x82 s']
      ...
      by (metis NoLockFpInv_def RLockInv_def reach_nolockfp reach_rlock state_km.distinct(33)
...
next
  ...
  next
    ...
    then show ?thesis using reach_trans
      apply (auto simp add: tps_trans_defs km_unchanged_defs WLockFpContentInv_def)
      apply (metis RLockInv_def reach_rlock state_km.distinct(41))
      apply (smt reach_wlock state_km.distinct(41) the_wr_tI)
...
next
  ...
  then show ?case apply (cases "not_tm_commit e") using kvs_of_gs_inv[of s e s']
    apply (auto simp add: TMFullView_def unchanged_defs cl_view_inv)
    using committed_kvs_view_grows[of s]
...
lemma vl_writers_sqns_reads_other_cl_inv:
  "vl_writers_sqns (update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t))) tSkm tFk vl) = 
   vl_writers_sqns vl"
  ...
  subgoal for cl x i apply (rule bexI[where x="update_kv_reads_all_txn
      (\<lambda>t. tm_state (cls s (get_cl_txn t))) tSkm tFk vl ! i"])
     apply (auto simp add: update_kv_reads_all_defs)
    by (metis (lifting) nth_list_update_eq nth_list_update_neq version.select_convs(2)
        version.surjective version.update_convs(3))
  done
...
lemma get_sqns_other_cl_inv:
  ...
  using assms by (auto simp add: get_sqns_def kvs_writers_sqns_other_cl_inv
      kvs_readers_sqns_other_cl_inv tm_unchanged_defs)
...
lemma get_sqns_tm_commit_grows:
  ...
  using assms
  apply (cases "\<forall>k. km_state (svrs s k) (get_txn_cl cl s) = no_lock")
...
next
  ...
  next
    ...
    then show ?case
      ...
      apply (metis state_tm.distinct(3))
      by (metis state_tm.distinct(7))
  next
    case (TM_Commit x1 x2 x3 x4)
    then show ?case
      apply (auto simp add: SqnInv_def tps_trans_defs) using get_sqns_other_cl_inv
      apply (smt (verit) tm_unchanged_defs reach_kvs_non_emp reach_nolockfp reach_rlock
          reach_rlockfp reach_tidfuturekm reach_tidpastkm reach_wlock reach_wlockfp)
      apply (cases "cl = x1")
      using get_sqns_tm_commit_grows[of s cl s']
      apply (smt (z3) tm_unchanged_defs UnE insert_absorb insert_iff insert_not_empty order.strict_implies_order
      ...
      by (smt (verit) tm_unchanged_defs reach_kvs_non_emp reach_nolockfp reach_rlock
          reach_rlockfp reach_tidfuturekm reach_tidpastkm reach_wlock reach_wlockfp)
  next
    case (TM_Abort x)
    then show ?case apply (auto simp add: SqnInv_def tps_trans_defs tm_unchanged_defs)
    apply (metis state_tm.distinct(7)) by (metis state_tm.distinct(11))
  next
    case (TM_ReadyC x)
    then show ?case apply (auto simp add: SqnInv_def tps_trans_defs tm_unchanged_defs)
    apply (metis less_Suc_eq_le) by (metis state_tm.distinct(3))
  next
    case (TM_ReadyA x)
    then show ?case apply (auto simp add: SqnInv_def tps_trans_defs tm_unchanged_defs)
    apply (metis less_Suc_eq state_tm.distinct(11)) by (metis state_tm.distinct(3))
...
lemma helper_lemma:
  assumes "i \<in> full_view (update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t)))
    (km_state (svrs s k)) (svr_key_fp (svrs s k)) (svr_vl (svrs s k)))"
  shows "
  update_kv_writes (the_wr_t (km_state (svrs s k))) (svr_key_fp (svrs s k) (the_wr_t (km_state (svrs s k))))
    (update_kv_reads_all_txn (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s k))
      (svr_key_fp (svrs s k)) (svr_vl (svrs s k))) ! i =
  update_kv_reads_all_txn (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s k))
    (svr_key_fp (svrs s k)) (svr_vl (svrs s k)) ! i"
  using assms
  update_kv_writes_version_inv[of i "update_kv_reads_all_txn (\<lambda>t. tm_state (cls s' (get_cl_txn t)))
   (km_state (svrs s k)) (svr_key_fp (svrs s k)) (svr_vl (svrs s k))" "the_wr_t (km_state (svrs s k))"
   "svr_key_fp (svrs s k) (the_wr_t (km_state (svrs s k)))"]
...
lemma kvs_of_gs_version_order:
  ...
  apply (auto simp add: kvs_of_gs_def km_tm_cl'_unchanged_def update_kv_all_tm_commit_no_lock_inv)
   ...
   using tm_commit_updates_kv_reads[of s s' cl k] 
     expanding_feature3'[where vl="update_kv_reads_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t)))
      (km_state (svrs s k)) (svr_key_fp (svrs s k)) (svr_vl (svrs s k))"]
   by (auto simp add: update_kv_reads_all_defs)
...
lemma new_writer:
  assumes "WLockInv s k" and "WLockFpInv s k"
    and "tm_state (cls s' cl) = tm_committed"
    and "km_state (svrs s k) (get_txn_cl cl s) = write_lock"
  shows "v_writer (update_kv_all_txn (\<lambda>t. tm_state (cls s' (get_cl_txn t))) (km_state (svrs s k))
    (svr_key_fp (svrs s k)) (svr_vl (svrs s k)) ! length (svr_vl (svrs s k))) =
   Tn (the_wr_t (km_state (svrs s k)))"
  using assms
  apply (auto simp add: update_kv_all_txn_def update_kv_writes_all_txn_def update_kv_writes_def
...
lemma kvs_of_gs_view_atomic:
  ...
  subgoal for k k' i i'
    ...
    apply (cases "i' \<in> full_view (update_kv_all_txn (\<lambda>t. tm_state (cls s (get_cl_txn t)))
      (km_state (svrs s k')) (svr_key_fp (svrs s k')) (svr_vl (svrs s k')))"; simp)
...
next
  ...
  then show ?case using p apply simp
    ...
    subgoal apply (auto simp add: fp_property_def view_snapshot_def)
      subgoal for k y
        apply (cases "km_state (svrs gs k) (get_txn_cl cl gs) = no_lock")
