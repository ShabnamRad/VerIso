...
text \<open>Extending N\<close>

lemma closed_general_mono_N:
...
text \<open>Extending V\<close>

lemma closed_general_mono_V:
  assumes "closed_general V r N" 
  and "V \<subseteq> V'" 
  shows "closed_general V' r N"
  using assms
...
lemma closed_general_union_V:                \<comment> \<open>just union...\<close>
  assumes "closed_general V\<^sub>1 r N"
      and "closed_general V\<^sub>2 r N"
  shows "closed_general (V\<^sub>1 \<union> V\<^sub>2) r N"
  using assms
  by (auto simp add: closed_general_def)

lemma closed_general_insert_V:
  assumes "closed_general V r N"
  and "r\<^sup>* `` {t} \<subseteq> insert t (V \<union> N)"
  (* and "t \<notin> N"; assumption not needed with alternative def! *) 
  shows "closed_general (insert t V) r N"
  using assms
  by (auto simp add: closed_general_def)
...
lemma closed_general_extend_rel_with_max:    \<comment> \<open>generalizes previous lemma\<close>
  assumes "closed_general V r N" 
...
lemma closed_general_union_V_extend_N:
  ...
  using assms
  by (auto simp add: closed_general_union_V closed_general_mono_N)
...
proof -
  have "closed_general (vis\<^sub>1 \<union> vis\<^sub>2) r r_only'" using assms(1-3) 
    by (simp add: closed_general_union_V_extend_N)
  then show ?thesis using assms(4-6)
    by (simp add: closed_general_extend_rel_with_max)
qed
...
lemma union_closed':
  ...
  by (auto simp add: closed'_generalize visTx'_union_distr visTx'_same_writers[of K']
           intro!: closed_general_union_V_extend_N)
...
lemma insert_wr_t_closed':
  assumes "closed' K u r"
    ...
    and "(r^-1)\<^sup>* `` {t} \<subseteq> insert t (visTx' K u \<union> read_only_Txs K)"
    and "read_only_Txs K' = read_only_Txs K"
    and "kvs_writers K' = insert t (kvs_writers K)"
    and "snd ` t_wr_deps = {t}"
  shows "closed' K' (u \<union> t_wr_deps) r"
  using assms
  by (auto simp add: closed'_generalize visTx'_new_writer intro!: closed_general_insert_V del: subsetI)
...
lemma insert_kt_to_deps_closed':
  assumes "closed' K deps r"
    ...
    and "(r^-1)\<^sup>* `` {t} \<subseteq> insert t (visTx' K deps \<union> read_only_Txs K)"
  shows "closed' K (insert (k, t) deps) r"
  using assms
  by (auto simp add: closed'_generalize visTx'_observes_t intro!: closed_general_insert_V)
